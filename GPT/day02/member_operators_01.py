...
print("对“当前角色有效”的最终结果使用 not 取反，得到“当前角色无效”：",
      not ((current_role in allowed_roles) and current_role not in disabled_roles))
...
print("输入是否为空字符串。", environments == "")
print("输入是否无效（为空，或者环境不有效）：",
      environments == "" or environments not in allowed_environments or environments in forbidden_environments)
