...
print("对“当前角色有效”的最终结果使用 not 取反，得到“当前角色无效”：", not ((current_role in allowed_roles) and current_role not in disabled_roles))
...
print("以下任意一个情况出现时，请求存在异常：状态码不是 200,响应文本包含error,响应时间大于 1000",
      status_code != 200 or "error"  in response_text or response_time > 1000)
