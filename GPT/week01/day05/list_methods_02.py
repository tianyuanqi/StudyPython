...
print("201是否在列表中:", 201 in status_codes)
...
sort_response_time = sorted(response_times)
print("列表改为升序:", sort_response_time)
...
print(f"最小响应时间为:{min(response_times)}ms" )
print(f"最大响应时间为:{max(response_times)}ms")
...
new_order = execution_order[::-1]
print("切片得到的新列表:", new_order)
print("原始列表为:", execution_order)
reverse_order = execution_order.reverse()
print("翻转列表为", reverse_order)
print("原始列表为", execution_order)
print("两种反转是否相等:", new_order == execution_order)
...
case_times = [125.5, 320.8, 98.2, 210.4, 150.0]
print("这是一份测试报告:")
print("用例数量:",len(case_times))
print(f"最快响应:{min(case_times):.2f}ms")
print(f"最慢响应:{max(case_times):.2f}ms")
print(f"平均响应:{sum(case_times) / len(case_times):.2f}ms")
