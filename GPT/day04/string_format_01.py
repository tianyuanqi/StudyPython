...
print(f"商品:{product_name}，单价:{unit_price:.2f}元，"
      f"数量:{quantity},商品总价为:{(unit_price * quantity):.2f}元")
...
pass_case = executed_cases - failed_cases
print(f"通过数量:{pass_case}")
print(f"失败数量:{failed_cases}")

no_executed_case = total_cases - executed_cases
print(f"未执行数量:{no_executed_case}")
progress = executed_cases / total_cases
print(f"执行进度:{progress:.2%}")

pass_rate = (executed_cases - failed_cases) / executed_cases
...
print(f"CASE_{case_number:03d} | {api_name:<10} | {status_code} | {response_time:.2f} ms")
case_number=35
print(f"CASE_{case_number:03d} | {api_name:<10} | {status_code} | {response_time:.2f} ms")
